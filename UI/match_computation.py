...
def read_expr_profile(file_name):
    ...
    Removes 3rd column if present and returns error message. 
...
def compute_distance(profile, sample_data):
    ...
    # CHECK IF there is any nan or inf in there:

    #is_nan = np.all(np.isnan(profile_levels))
    #is_nan2 = np.all(np.isnan(TCGA_levels)) # There are nans in the kidney cancer profile.
    # for level in TCGA_levels:
    #     if np.isnan(level):
    #         print("THIS IS nan: ", level)


    #return("IS THERE is_nan?", is_nan, is_nan2)
    ...
    r,p = pearsonr(breast_values, kidney_values)
